...
nil の値を安全に処理できず、予期しないエラーの原因になる。
...
**もしこう書かなかったら：**
フィルター名を文字列で直接管理すると、入力ミスが起きやすくなる。例えば "sepia" と "seipa" のような間違いがあっても、コンパイル時に気づきにくい。$selectedItem のように Binding を使うことで、選択状態が変化した時に自動で状態更新される。
